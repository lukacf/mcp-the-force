...
async def test_specific_files():
    ...
    for file_path in existing_files[:5]:
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                print(f"   ✓ {Path(file_path).name}: {file_obj.id} in {time.time()-start:.2f}s")
    ...
    if file_ids:
        ...
        for i in range(10):
            ...
            print(f"   [{i+1}s] Status: {batch.status}, Completed: {batch.file_counts.completed}/{batch.file_counts.total}")
...
async def test_with_timeout_wrapper():
    ...
    try:
        ...
        print(f"Timeout after 30s! Cancelling...")
        task.cancel()
        try:
            await task
        except asyncio.CancelledError:
