...
class OpenAICleanupManager:
    ...
    async def get_stats(self) -> Tuple[int, int]:
        ...
        while has_more and file_count < 10000:
            if last_id:
                files_response = await self.client.files.list(purpose="assistants", limit=100, after=last_id)
            else:
                files_response = await self.client.files.list(purpose="assistants", limit=100)
    ...
    async def monitor_mode(self):
        ...
        while True:
            try:
                ...
                file_color = GREEN if (isinstance(file_count, int) and file_count < 5000) else YELLOW if (isinstance(file_count, int) and file_count < 10000) else RED
                ...
                if last_file_count is not None and isinstance(file_count, int) and isinstance(last_file_count, int):
                ...
                print(f"\r[{timestamp}] VS: {vs_color}{vs_count}/100{RESET}{vs_change} | "
                      f"Files: {file_color}{file_count}{RESET}{file_change}", end='', flush=True)
                
                # Warnings on new line
                if vs_count >= 99:
                    print(f"\n{RED}🚨 CRITICAL: At vector store limit - creation will hang!{RESET}", end='')
                elif vs_count >= 95:
                    print(f"\n{YELLOW}⚠️  WARNING: Approaching vector store limit!{RESET}", end='')
        ...
        for batch_start in range(0, len(stores_to_delete), batch_size):
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    failed += 1
                    print(f"{RED}✗ Failed to delete {batch[i].id[:16]}: {result}{RESET}")
                else:
                    deleted += 1
    ...
    async def cleanup_files(self, max_files: int = 1000, batch_size: int = 100):
        ...
        while True:
            ...
            try:
                if last_id:
                    response = await self.client.files.list(purpose="assistants", limit=batch_size, after=last_id)
                else:
                    response = await self.client.files.list(purpose="assistants", limit=batch_size)
            except Exception as e:
            ...
            if deleted > 0 and deleted % 500 == 0:
                count_response = await self.client.files.list(purpose="assistants", limit=1)
                if not count_response.has_more or (len(count_response.data) == 1 and deleted >= 1):
            ...
            print(f"\rBatch {batch_num} | Deleted: {deleted} | Failed: {failed} | "
                  f"Rate: {rate:.0f} files/sec | Elapsed: {int(elapsed)}s", end='', flush=True)
            ...
            if max_files == 0:
                ...
                if deleted > 10000:
                    ...
                    actual_count = 0
                    check_response = await self.client.files.list(purpose="assistants", limit=100)
    ...
    async def interactive_menu(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                print(f"{YELLOW}⚠️  Emergency cleanup will delete 50% of vector stores!{RESET}")
...
async def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "monitor":
            ...
            print("  python openai_cleanup_manager.py cleanup-vs [target]  # Cleanup vector stores")
            print("  python openai_cleanup_manager.py cleanup-files [max] [batch_size]  # Cleanup files")
            print("\nExamples:")
            print("  python openai_cleanup_manager.py cleanup-files 100  # Keep 100 files, 100 parallel")
            print("  python openai_cleanup_manager.py cleanup-files 0 200  # Delete ALL files, 200 parallel")
    else:
