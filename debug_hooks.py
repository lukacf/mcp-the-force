...
debug_handler.setFormatter(
    logging.Formatter('%(asctime)s.%(msecs)03d - %(name)s - %(message)s', 
                      datefmt='%H:%M:%S')
)
...
def trace_task_cancel(self, msg=None):
    ...
    debug_log(f"  Called from: {stack[-2].filename}:{stack[-2].lineno} in {stack[-2].name}")
...
async def trace_wait_for(fut, timeout):
    ...
    try:
        ...
        debug_log(f"WAIT_FOR completed successfully")
        return result
    except asyncio.TimeoutError:
        debug_log(f"WAIT_FOR timed out after {timeout}s")
        raise
    except asyncio.CancelledError:
        debug_log(f"WAIT_FOR was cancelled")
        raise
