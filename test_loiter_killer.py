...
class TestLoiterKillerService:
    ...
    @pytest.mark.asyncio
    async def test_service_starts_and_responds_to_health_check(self, loiter_killer_service):
    ...
    @pytest.mark.asyncio
    async def test_acquire_creates_new_vector_store_for_new_session(self, loiter_killer_service):
        """Test that acquire creates a new vector store for a new session."""
        async with httpx.AsyncClient() as client:
            session_id = "test_session_123"
            
            # Make request
            response = await client.post(f"http://localhost:9876/session/{session_id}/acquire")
    ...
    @pytest.mark.asyncio
    async def test_acquire_returns_existing_store_for_known_session(self, loiter_killer_service):
        """Test that acquire returns existing vector store for known session."""
        async with httpx.AsyncClient() as client:
            session_id = "test_session_456"
            
            # First request - create new
            response1 = await client.post(f"http://localhost:9876/session/{session_id}/acquire")
            ...
            response2 = await client.post(f"http://localhost:9876/session/{session_id}/acquire")
    ...
    async def test_file_tracking(self, loiter_killer_service):
        ...
        async with httpx.AsyncClient() as client:
            ...
            response3 = await client.post(f"http://localhost:9876/session/{session_id}/acquire")
            assert response3.status_code == 200
            data3 = response3.json()
            assert data3["reused"] is True
            assert set(data3["files"]) == set(file_ids)
    ...
    async def test_lease_renewal(self, loiter_killer_service):
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Renew the lease
            response2 = await client.post(f"http://localhost:9876/session/{session_id}/renew")
            assert response2.status_code == 200
            assert response2.json()["status"] == "renewed"
            
            # Try to renew non-existent session
            response3 = await client.post(f"http://localhost:9876/session/non_existent/renew")
    ...
    async def test_cleanup_expired_sessions(self, loiter_killer_service):
        ...
        async with httpx.AsyncClient() as client:
            ...
            response1 = await client.post(f"http://localhost:9876/session/{session_id}/acquire")
            assert response1.status_code == 200
            vector_store_id = response1.json()["vector_store_id"]
            
            # Track some files
            file_ids = ["cleanup_file_001", "cleanup_file_002"]
            response2 = await client.post(
                f"http://localhost:9876/session/{session_id}/files",
                json=file_ids
            )
            ...
            response4 = await client.post(f"http://localhost:9876/session/{session_id}/renew")
    ...
    async def test_error_handling(self, loiter_killer_service):
        ...
        async with httpx.AsyncClient() as client:
            ...
            response1 = await client.post(
                "http://localhost:9876/session/non_existent/files",
                json=["file_001"]
            )
            assert response1.status_code == 404
            
            # Test renewing non-existent session (already tested above)
            response2 = await client.post("http://localhost:9876/session/non_existent/renew")
