...
def traced_cancel(self, msg=None):
    ...
    coro_name = self.get_coro().__name__ if hasattr(self.get_coro(), '__name__') else str(self.get_coro())
...
async def traced_wait_for(coro, timeout):
    ...
    try:
        ...
        elapsed = time.time() - start_time
        trace_log(f"wait_for TIMED OUT after {elapsed:.3f}s (timeout was {timeout}s)", "TIMEOUT")
        raise
...
from mcp_second_brain.operation_manager import operation_manager, OperationManager

# Patch run_with_timeout
original_run_with_timeout = OperationManager.run_with_timeout

async def traced_run_with_timeout(self, operation_id: str, coro: Coroutine, timeout: int):
    ...
    try:
        ...
        trace_log(f"Operation {operation_id} failed with {type(e).__name__}: {e}", "ERROR")
        raise
...
try:
    ...
    if hasattr(FastMCP, '_send_response'):
        ...
        async def traced_send_response(self, *args, **kwargs):
            trace_log(f"FastMCP._send_response called with args={args}, kwargs={kwargs}", "RESPONSE")
            try:
                result = await original_send_response(self, *args, **kwargs)
                trace_log("FastMCP._send_response completed successfully", "RESPONSE")
                return result
            except Exception as e:
                trace_log(f"FastMCP._send_response failed with {type(e).__name__}: {e}", "ERROR")
                raise
...
async def traced_execute_tool(self, tool_id: str, arguments: dict[str, Any]):
    ...
    try:
        ...
        trace_log(f"Tool execution failed: {tool_id} - {type(e).__name__}: {e}", "ERROR")
        raise
