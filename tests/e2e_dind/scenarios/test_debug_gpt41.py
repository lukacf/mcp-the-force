...
def test_debug_gpt41(claude):
    ...
    response = claude(
        "Use the-force chat_with_gpt41 with instructions: 'Say hello', output_format: 'Just say hello', context: [], session_id: 'debug-test'"
    )
    ...
    response = claude(
        "Use chat_with_gpt41 with instructions: 'Say hello', output_format: 'Just say hello', context: [], session_id: 'debug-test-2'"
    )
    print(f"Response: {response}")
