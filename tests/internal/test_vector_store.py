...
class TestVectorStoreIntegration:
    ...
    async def test_vector_store_creation(self, tmp_path, mock_env, mock_openai_client):
        ...
        mock_openai_client.vector_stores.file_batches.upload_and_poll.return_value = (
            Mock(status="completed", file_counts=Mock(completed=3, failed=0))
        )
    ...
    async def test_vector_store_with_attachments(
        self, temp_project, mock_openai_client
    ):
    ...
    async def test_vector_store_file_filtering(
        self, tmp_path, mock_env, mock_openai_client
    ):
        ...
        mock_openai_client.vector_stores.file_batches.upload_and_poll.side_effect = (
            mock_upload
        )
    ...
    async def test_vector_store_error_handling(self, tmp_path, mock_openai_client):
        ...
        mock_openai_client.vector_stores.create.side_effect = Exception(
            "VS creation failed"
        )
    ...
    async def test_large_attachment_handling(self, tmp_path, mock_openai_client):
        ...
        mock_openai_client.vector_stores.file_batches.upload_and_poll.return_value = (
            Mock(status="completed", file_counts=Mock(completed=100))
        )
