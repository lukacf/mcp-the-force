...
class TestMemoryConfig:
    ...
    def test_init_creates_config_directory(self, temp_config_dir, mock_client):
        ...
        with patch("mcp_second_brain.memory.config.get_client", return_value=mock_client):
    ...
    def test_load_existing_config(self, temp_config_dir, mock_client):
        ...
        existing_config = {
            "conversation_stores": [{"id": "conv-1", "count": 50, "created": "2024-01-01"}],
    ...
    def test_get_active_conversation_store_creates_first(self, temp_config_dir, mock_client, mock_settings):
        """Test creating first conversation store."""
        config_path = temp_config_dir / "stores.json"
        
        with patch("mcp_second_brain.memory.config.get_client", return_value=mock_client):
            with patch("mcp_second_brain.memory.config.get_settings", return_value=mock_settings):
                config = MemoryConfig(config_path)
        ...
        assert len(config._config["conversation_stores"]) == 1
        assert config._config["active_conv_index"] == 0
        mock_client.vector_stores.create.assert_called_once_with(name="project-conversations-001")

    def test_get_active_conversation_store_rollover(self, temp_config_dir, mock_client, mock_settings):
        """Test store rollover when limit reached."""
        config_path = temp_config_dir / "stores.json"
        
        # Create config with store at limit
        existing_config = {
            "conversation_stores": [{"id": "conv-1", "count": 100, "created": "2024-01-01"}],
            "commit_stores": [],
            "active_conv_index": 0,
        ...
        assert config._config["active_conv_index"] == 1
        mock_client.vector_stores.create.assert_called_once_with(name="project-conversations-002")
    ...
    def test_increment_counts(self, temp_config_dir, mock_client):
        ...
        existing_config = {
            "conversation_stores": [{"id": "conv-1", "count": 10, "created": "2024-01-01"}],
    ...
    def test_get_all_store_ids(self, temp_config_dir, mock_client):
        ...
        existing_config = {
            ...
            "commit_stores": [
                {"id": "commit-1", "count": 5, "created": "2024-01-01"}
            ],
            "active_conv_index": 1,
            "active_commit_index": 0,
            "created_at": "2024-01-01",
