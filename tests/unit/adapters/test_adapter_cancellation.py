...
async def test_grok_adapter_propagates_cancellation():
    ...
    with patch("mcp_second_brain.adapters.grok.adapter.get_settings") as mock_settings:
        ...
        with patch("mcp_second_brain.adapters.grok.adapter.AsyncOpenAI") as mock_client_class:
...
async def test_openai_adapter_propagates_cancellation():
    ...
    with patch("mcp_second_brain.adapters.openai.adapter.OpenAIClientFactory") as mock_factory:
        mock_client = AsyncMock()
        mock_factory.get_client.return_value = mock_client
        
        # Make the orchestrator raise CancelledError
        with patch("mcp_second_brain.adapters.openai.adapter.FlowOrchestrator") as mock_orchestrator_class:
