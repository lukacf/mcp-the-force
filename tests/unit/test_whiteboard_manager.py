...
class TestWhiteboardManagerCreation:
    ...
    @pytest.mark.asyncio
    async def test_create_whiteboard_openai_first(self, whiteboard_manager, mock_vector_store_manager):
    ...
    @pytest.mark.asyncio 
    async def test_create_whiteboard_fallback_to_hnsw(self, whiteboard_manager, mock_vector_store_manager):
    ...
    @pytest.mark.asyncio
    async def test_create_whiteboard_stores_metadata(self, whiteboard_manager, mock_unified_session_cache):
        """Test that store info is saved to session metadata."""
        
        # Mock successful creation
        whiteboard_manager.vs_manager.create = AsyncMock(return_value={
            "store_id": "vs_test_789",
            "provider": "openai"
        })
        ...
        mock_unified_session_cache.set_metadata.assert_called_once_with(
            ...
            key="whiteboard",
            value={"store_id": "vs_test_789", "provider": "openai"}
        )
...
class TestWhiteboardManagerMessages:
    ...
    async def test_append_message_creates_vsfile(self, whiteboard_manager):
        ...
        # Setup: Mock existing store info
        store_info = {"store_id": "vs_test_123", "provider": "openai"}
        whiteboard_manager.session_cache.get_metadata = AsyncMock(return_value=store_info)
        
        # Mock VSFile addition
        mock_store = Mock()
        mock_store.add_files = AsyncMock()
        whiteboard_manager.vs_manager.get_store = Mock(return_value=mock_store)
...
class TestWhiteboardManagerStoreInfo:
    ...
    @pytest.mark.asyncio
    async def test_get_store_info_from_session_metadata(self, whiteboard_manager, mock_unified_session_cache):
        ...
        # Verify correct metadata key was requested
    ...
    @pytest.mark.asyncio
    async def test_get_store_info_returns_none_if_missing(self, whiteboard_manager, mock_unified_session_cache):
    ...
    async def test_get_or_create_store_reuses_existing(self, whiteboard_manager):
        ...
        whiteboard_manager.session_cache.get_metadata = AsyncMock(return_value=existing_info)
...
class TestWhiteboardManagerSummarization:
    ...
    async def test_summarize_and_rollover_new_store(self, whiteboard_manager):
        ...
        # Mock current store info
        old_store_info = {"store_id": "vs_old_full", "provider": "hnsw"}
        whiteboard_manager.session_cache.get_metadata = AsyncMock(return_value=old_store_info)
        
        # Mock session messages for summarization
        mock_session = Mock()
        mock_session.messages = [Mock() for _ in range(60)]  # Over threshold
        whiteboard_manager.session_cache.get_session = AsyncMock(return_value=mock_session)
        
        # Mock summarization service
        with patch('mcp_the_force.local_services.describe_session.DescribeSessionService') as mock_describe:
            mock_describe.return_value.execute = AsyncMock(return_value="Collaboration summary")
            
            # Mock new store creation
            new_store_info = {"store_id": "vs_new_rolled", "provider": "hnsw"}
            whiteboard_manager.vs_manager.create = AsyncMock(return_value=new_store_info)
            
            # Mock the new store's add_files method
            mock_new_store = Mock()
            mock_new_store.add_files = AsyncMock()
            whiteboard_manager.vs_manager.get_store = Mock(return_value=mock_new_store)
            
            await whiteboard_manager.summarize_and_rollover("rollover-test", threshold=50)
            
            # Verify old store marked inactive
            whiteboard_manager.vs_manager.vector_store_cache.set_inactive.assert_called_once_with("vs_old_full")
            
            # Verify new store created
            whiteboard_manager.vs_manager.create.assert_called_once()
            
            # Verify new store info saved to metadata
            whiteboard_manager.session_cache.set_metadata.assert_called_with(
                tool="chatter_collaborate",
                session_id="rollover-test", 
                key="whiteboard",
                value=new_store_info
            )
...
class TestWhiteboardManagerErrors:
    ...
    @pytest.mark.asyncio
    async def test_create_whiteboard_both_providers_fail(self, whiteboard_manager, mock_vector_store_manager):
    ...
    @pytest.mark.asyncio
    async def test_renew_lease_called(self, whiteboard_manager, mock_vector_store_manager):
        ...
        mock_vector_store_manager.renew_lease.assert_called_once_with("collab_lease-test")