...
from mcp_the_force.local_services.chatter_progress_installer import (
    ChatterProgressInstaller,
)
...
class TestChatterProgressInstaller:
    ...
    async def test_install_dry_run(self, installer, temp_project):
        ...
        with patch("mcp_the_force.config.get_settings") as mock_settings:
            ...
            result = await installer.execute(
                action="install", project_dir=str(temp_project), dry_run=True
            )
    ...
    async def test_status_no_installation(self, installer, temp_project):
        ...
        with patch("mcp_the_force.config.get_settings") as mock_settings:
            ...
            result = await installer.execute(
                action="status", project_dir=str(temp_project)
            )
    ...
    async def test_unknown_action(self, installer, temp_project):
        ...
        result = await installer.execute(
            action="invalid_action", project_dir=str(temp_project)
        )
    ...
    async def test_existing_status_line_detection(self, installer, temp_project):
        ...
        existing_settings = {
            "statusLine": {"type": "command", "command": "echo 'My Custom Status'"}
        }
    ...
    async def test_settings_merging_preserves_existing(self, installer, temp_project):
        ...
        assert (
            updated_settings["hooks"]["PreToolUse"][0]["matcher"] == "existing_matcher"
        )
