...
async def test_post_abort():
    ...
    try:
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] Response reader cancelled (Claude stops listening)"
        )
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] But stdin still open (Claude might send more)"
        )
        ...
        list_tools = {"jsonrpc": "2.0", "method": "tools/list", "params": {}, "id": 3}
        ...
        try:
            ...
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Failed to send: {type(e).__name__}: {e}"
            )
        ...
        try:
            ...
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Got response: {response.decode().strip()[:100]}"
            )
        except asyncio.TimeoutError:
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] No response after 5 seconds - server may be frozen"
            )
        except Exception as e:
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Read failed: {type(e).__name__}: {e}"
            )

        # Try another command - maybe a simpler one
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Trying a simpler command...")
        ping = {"jsonrpc": "2.0", "method": "ping", "params": {}, "id": 4}
        ...
        if proc.returncode is None:
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] Server still running (PID {proc.pid})"
            )
        else:
            print(
