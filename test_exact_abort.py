...
async def simulate_claude_abort():
    ...
    try:
        ...
        print(
            f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Got response: {response.decode().strip()[:100]}..."
        )

        # Send initialized notification (Claude does this)
        initialized_notif = {"jsonrpc": "2.0", "method": "notifications/initialized"}
        print(
            f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Sending initialized notification..."
        )
        ...
        async def response_reader():
            try:
                ...
                print(
                    f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Response reader cancelled"
                )
                raise
        ...
        for i in range(10):
            ...
            if proc.returncode is not None:
                print(
                    f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Server exited with code {proc.returncode}"
                )
                break
            else:
                print(
                    f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] Server still running (PID {proc.pid})..."
                )
