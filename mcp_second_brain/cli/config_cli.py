"""Configuration management CLI for mcp-second-brain."""
...
from mcp_second_brain.config import get_settings, CONFIG_FILE, SECRETS_FILE

app = typer.Typer(help="MCP Second-Brain configuration management")

# Sensitive key names to mask
SENSITIVE_KEY_NAMES = ["api_key", "password", "secret", "token"]

# Template for initial config.yaml
INIT_CONFIG_TEMPLATE = """# MCP Second-Brain Configuration
...
INIT_SECRETS_TEMPLATE = """# MCP Second-Brain Secrets Configuration
...
def import_legacy(
    ...
    if mcp_config and mcp_config.exists():
        ...
        if "mcpServers" in mcp_data and "second-brain" in mcp_data["mcpServers"]:
            server_env = mcp_data["mcpServers"]["second-brain"].get("env", {})
