...
class VertexAdapter(BaseAdapter):
    ...
    async def generate(self, prompt: str, vector_store_ids: List[str] | None = None,
        ...
        file_search = None
        if vector_store_ids:
            logger.info(f"Registering file_search for {len(vector_store_ids)} vector stores")
        ...
        if file_search and response.candidates:
            return await self._handle_function_calls(response, contents, generate_content_config, file_search)
    ...
    async def _handle_function_calls(self, response: Any, contents: List[types.Content], 
                                    config: types.GenerateContentConfig, 
                                    file_search: GeminiFileSearch) -> str:
