...
# Factory pattern - try SQLite first, fall back to in-memory
try:
    _instance = _SQLiteSessionCache()
except Exception as exc:
    logger.warning(f"Falling back to in-memory session cache: {exc}")
    _instance = _InMemorySessionCache()
...
atexit.register(SessionCache.close)