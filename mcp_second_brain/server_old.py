...
from .model_config import load_models, ModelConfig
...
def create_tool_function(cfg: ModelConfig):
    ...
    # Build the function with dynamic parameters based on model capabilities
    if cfg.supports_vector_store:
        # OpenAI models: include attachments parameter
        async def tool_function(
            ...
            return await _execute_tool(cfg, instructions, output_format, context, 
                                     attachments, temperature, reasoning_effort, 
                                     max_reasoning_tokens, session_id)
    else:
        # Gemini models: no attachments parameter
        async def tool_function(
            instructions: str,
            output_format: str,
            context: List[str],
            ...
            return await _execute_tool(cfg, instructions, output_format, context, 
                                     None, temperature, None, None, session_id)
...
async def _execute_tool(
    ...
    session_id: Optional[str]
        ...
        if temperature is not None and cfg.supports_temperature:
