    ...
    try:
        ...
        async def respond_safe(self, response: Any) -> None:
            ...
            try:
                ...
                logger.info(f"Cannot send response - client disconnected: {type(e).__name__}")
                # Don't re-raise - let the request complete cleanly
            except Exception as e:
                # Check for other disconnect-related errors by name
                error_name = type(e).__name__.lower()
                if any(x in error_name for x in ['broken', 'closed', 'pipe', 'connection', 'cancelled']):
                    logger.info(f"Cannot send response - client disconnected: {type(e).__name__}")
                    return
        ...
        logger.info("Successfully patched RequestResponder.respond for disconnect handling")
...
def patch_request_responder_cancel():
    ...
    try:
        ...
        logger.info("Successfully patched RequestResponder.cancel to prevent double responses")
