...
class StateResetManager:
    ...
    async def reset_all_state(self):
        ...
        if tasks_to_cancel:
            logger.warning(f"[STATE RESET] Cancelling {len(tasks_to_cancel)} pending tasks")
        ...
        for singleton_dict in self._singletons_to_clear:
            try:
                logger.warning(f"[STATE RESET] Clearing singleton: {type(singleton_dict).__name__}")
    ...
    async def _clear_openai_clients(self):
        ...
        try:
            ...
            if hasattr(OpenAIClientFactory, '_instances'):
                logger.warning(f"[STATE RESET] Clearing {len(OpenAIClientFactory._instances)} OpenAI client instances")
            ...
            if hasattr(OpenAIClientFactory, '_instances'):
                for client in list(OpenAIClientFactory._instances.values()):
                    try:
                        ...
                        logger.debug(f"[STATE RESET] Failed to close OpenAI client: {e}")
    ...
    async def _reset_thread_pool(self):
        ...
        try:
            ...
            if hasattr(thread_pool, '_shared_executor') and thread_pool._shared_executor:
                    ...
                    for attr_name, attr_value in module.__dict__.items():
                        if hasattr(attr_value, 'cache_clear'):
                            try:
                                ...
                                logger.debug(f"[STATE RESET] Cleared cache for {module.__name__}.{attr_name}")
                            except:
                                pass
        except Exception as e:
    ...
    async def wrap_tool_execution(self, tool_func, *args, **kwargs):
        ...
        try:
            ...
            logger.warning(f"[STATE RESET] Tool execution #{self._execution_count} complete, resetting state")
