...
SECRET_PATTERNS = [
    # API Keys
    (r'(api[_-]?key|apikey|access[_-]?key)[\s:=]+[\'""]?([a-zA-Z0-9_\-]{20,})[\'""]?', r'\1=REDACTED'),
    (r'(secret[_-]?key|secret)[\s:=]+[\'""]?([a-zA-Z0-9_\-]{20,})[\'""]?', r'\1=REDACTED'),
    
    # AWS
    (r'(AKIA[A-Z0-9]{16})', 'AWS_ACCESS_KEY_REDACTED'),
    (r'(aws[_-]?secret[_-]?access[_-]?key)[\s:=]+[\'""]?([a-zA-Z0-9/+=]{40})[\'""]?', r'\1=REDACTED'),
...
def redact_dict(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        if isinstance(value, str):
            ...
            result[key] = [
                redact_secrets(item) if isinstance(item, str) else
