...
class CollaborationService:
    ...
    async def execute(self,
        ...
        logger.info(f"Starting collaboration session {session_id} with {len(models)} models")
        ...
        try:
            ...
            logger.debug(f"Using whiteboard {whiteboard_info['store_id']} for session {session_id}")
            ...
            if len(session.messages) >= config.summarization_threshold:
                logger.info(f"Triggering summarization for session {session_id} ({len(session.messages)} messages)")
                await self.whiteboard.summarize_and_rollover(session_id, config.summarization_threshold)
            ...
            logger.info(f"Starting collaboration loop: current_step={session.current_step}, max_steps={session.max_steps}")
            
            while not session.is_completed():
                # Safety: don't exceed configured max steps
                if session.current_step >= session.max_steps:
                    logger.info(f"Reached max steps ({session.max_steps}) for session {session_id}")
                    break
                ...
                response = await self._execute_model_turn(session, whiteboard_info, config.timeout_per_step)
                ...
                logger.info(f"Completed turn {steps_ran} for session {session_id} ({next_model})")
            ...
            logger.error(f"Collaboration execution failed for session {session_id}: {e}")
            ...
            try:
                ...
                if collab_state:
                    ...
                    await self.session_cache.set_metadata(
                        project, "chatter_collaborate", session_id, "collab_state", collab_state
                    )
            except Exception:
    ...
    async def _get_or_create_session(self,
                                   ...
                                   models: list[str],
                                   mode: str,
                                   config: CollaborationConfig) -> CollaborationSession:
        ...
        if existing_state:
            ...
            return CollaborationSession.from_dict(existing_state)
    ...
    async def _execute_model_turn(self, 
        ...
        to provide models with access to the shared whiteboard.
        ...
        try:
            ...
            if timeout is not None:
                logger.debug(f"Enforcing {timeout}s timeout for {next_model} in session {session.session_id}")
            ...
            response = await self.executor.execute(
                ...
                vector_store_ids=[whiteboard_info["store_id"]],  # Critical: Whiteboard access via passthrough
                timeout=timeout  # Pass timeout as kwarg for executor override
            )
            
            logger.debug(f"Successfully executed {next_model} for session {session.session_id}")
            return response
            
        except Exception as e:
            logger.error(f"Model execution failed for {next_model} in session {session.session_id}: {e}")
            
            # Return error message but don't crash the collaboration
            return f"Error from {next_model}: {str(e)}"

    def _build_collaboration_instructions(self, session: CollaborationSession, model_name: str) -> str:
