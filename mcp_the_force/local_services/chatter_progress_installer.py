...
class ChatterProgressInstaller:
    ...
    async def _install(self, project_path: Path, with_hooks: bool, dry_run: bool) -> str:
        ...
        # 5. Create disable sentinel support
        ...
        results.append("2. Try a Chatter collaboration - progress will appear in status line")
        results.append(f"3. To disable: touch {chatter_dir}/disable_statusline")
        results.append(f"4. To uninstall: call install_chatter_progress(action='uninstall')")
            ...
            if settings_file.exists():
                try:
                    ...
                    if "statusLine" in settings:
                        ...
                        if isinstance(status_config, dict) and "command" in status_config:
                            return status_config["command"]
    ...
    async def _install_status_line_mux(
        ...
        if existing_cmd:
            ...
            return "✅ Created status line multiplexer (no existing status line detected)"
    ...
    async def _update_settings_file(
        ...
        mux_path = f"./.claude/chatter/statusline_mux.sh"
        settings["statusLine"] = {
            "type": "command",
            "command": mux_path
        }
    ...
    def _merge_chatter_hooks(self, hooks_config: Dict[str, Any], chatter_dir: Path) -> None:
        ...
        chatter_pre_hook = {
            "matcher": "mcp__the-force__chatter_collaborate",
            "hooks": [
                {
                    "type": "command",
                    "command": f"./.claude/chatter/pre_chatter.sh",
                    "timeout": 5
                }
            ]
        }
        ...
        if not any(
            hook.get("matcher") == "mcp__the-force__chatter_collaborate" 
        ...
        chatter_post_hook = {
            ...
            "hooks": [
                {
                    "type": "command", 
                    "command": f"./.claude/chatter/post_chatter.sh",
                    "timeout": 10
                }
            ]
        }
        ...
        chatter_stop_hook = {
            ...
            "hooks": [
                {
                    "type": "command",
                    "command": f"./.claude/chatter/stop_chatter.sh", 
                    "timeout": 5
                }
            ]
        }
...
if [[ -f "$progress_file" ]]; then
    ...
    if [[ -n "$current_model" ]]; then
        model_short=$(echo "$current_model" | sed 's/chat_with_//' | sed 's/gpt/GPT-/' | sed 's/gemini25/Gemini/' | sed 's/claude41/Claude/')
    fi
...
'''
    ...
    async def _uninstall(self, project_path: Path, dry_run: bool) -> str:
        ...
        if backup_file.exists():
            ...
            results.append(f"✅ Restored original settings from backup")
