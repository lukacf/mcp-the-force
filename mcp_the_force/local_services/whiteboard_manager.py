...
class WhiteboardManager:
    ...
    async def create_whiteboard(self, session_id: str) -> Dict[str, str]:
        ...
        try:
            logger.debug(
                f"Creating whiteboard with OpenAI provider for session {session_id}"
            )
            ...
            if isinstance(result, dict):
                ...
                logger.info(
                    f"Created {provider} whiteboard store {result['store_id']} for session {session_id}"
                )
            else:
            ...
            try:
                ...
                result = await self.vs_manager.create(
                    files=[], session_id=collab_session_id, provider="hnsw"
                )
                ...
                logger.error(
                    f"Both OpenAI and HNSW whiteboard creation failed for {session_id}: {hnsw_error}"
                )
                raise
    ...
    async def get_or_create_store(self, session_id: str) -> Dict[str, str]:
        ...
        if store_info:
            logger.debug(
                f"Reusing existing whiteboard {store_info['store_id']} for session {session_id}"
            )
            return store_info
    ...
    async def append_message(
        ...
        vsfile = VSFile(
            path=vsfile_path, content=vsfile_content, metadata=vsfile_metadata
        )
        ...
        logger.debug(
            f"Appended message {message_idx} from {message.speaker} to whiteboard {store_info['store_id']}"
        )
    ...
    async def summarize_and_rollover(self, session_id: str, threshold: int) -> None:
        ...
        if not collab_state or len(collab_state.get("messages", [])) < threshold:
            logger.debug(
                f"Session {session_id} under threshold ({len(collab_state.get('messages', [])) if collab_state else 0} < {threshold}), skipping rollover"
            )
            return

        logger.info(
            f"Starting summarization and rollover for session {session_id} ({len(collab_state.get('messages', []))} messages)"
        )
        ...
        try:
            ...
            await self.vs_manager.vector_store_cache.set_inactive(
            ...
            logger.info(
                f"Completed rollover for session {session_id}: {old_store_info['store_id']} -> {new_store_info['store_id']}"
            )
