...
async def check_relationship():
    ...
    print(f"Checking first 10 vector stores...")
    print("=" * 60)
    ...
    for vs in vs_response.data:
        ...
        if vs.file_counts.total > 0:
            ...
            try:
                files = await client.vector_stores.files.list(vector_store_id=vs.id, limit=5)
                print(f"Sample files ({len(files.data)} shown of {vs.file_counts.total}):")
                for f in files.data:
                    print(f"  - {f.id}")
