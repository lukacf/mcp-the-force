...
async def test_aggressive_abort():
    ...
    for method in abort_methods:
        ...
        try:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Aborting with method: {method}")
            ...
            try:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] Server still running after 10s")
                proc.terminate()
                await proc.wait()
            ...
            for line in stderr_lines:
                if any(x in line.lower() for x in ['error', 'exception', 'traceback', 'assert']):
                    if not error_found:
                        print("\n*** ERRORS DETECTED ***")
                        error_found = True
                    print(line.rstrip())
                ...
                with open("mcp_cancellation_debug.log", "r") as f:
                    ...
                    if "ExceptionGroup" in content:
                        ...
                        for line in content.split('\n'):
                            if any(x in line for x in ['ExceptionGroup', 'CANCEL', 'already']):
                                print(line)
