...
def signal_handler(signum, frame):
    ...
    debug_break = True
    print(
        "\n[DEBUGGER] Signal received! Will break at next opportunity...",
        file=sys.stderr,
    )
...
operation_manager.operation_manager.run_with_timeout = debug_run_with_timeout.__get__(
    operation_manager.operation_manager, operation_manager.OperationManager
)

print(f"[DEBUGGER] Process PID: {os.getpid()}", file=sys.stderr)
print(
    f"[DEBUGGER] Send 'kill -USR1 {os.getpid()}' to break into debugger",
    file=sys.stderr,
)
print("[DEBUGGER] Debugger injection complete. Starting server...", file=sys.stderr)
