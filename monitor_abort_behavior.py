                        ...
                        if current_size > last_size:
                            with open(filepath, 'r') as f:
                                ...
                                if new_content.strip():
                                    self.log_event('DEBUG', f"{label}: {new_content.strip()[:200]}")
    ...
    try:
        ...
        for i in range(30):
            ...
            if proc.returncode is not None:
                monitor.log_event('SERVER', f"Process exited with code {proc.returncode}")
                break
            else:
                if i % 5 == 0:
        ...
        # Key timings
        abort_time = next((e['elapsed'] for e in monitor.events if e['type'] == 'ABORT'), None)
        exit_time = next((e['elapsed'] for e in monitor.events if 'exited' in e['message']), None)
